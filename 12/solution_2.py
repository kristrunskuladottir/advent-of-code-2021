...
def construct_ways(cave_map: List[List[str]], unfinished_ways: List[List[str]]) -> List[List[str]]:
    finished_ways = []
    small_caves = [cave for cave in cave_map.keys() if cave.lower() == cave]
    ...
    while len(unfinished_ways) > 0:
        ...
        for next_cave in next_caves:
            ...
            if next_cave == 'end':
                ...
                new_way.append(next_cave)
                unfinished_ways.append(new_way)
            elif next_cave.lower() == next_cave and all(len([cave for cave in new_way if cave == small_cave]) <= 1 for small_cave in small_caves):
                new_way.append(next_cave)
                unfinished_ways.append(new_way)
